    ...
    <div class="container px-lg-5 mt-lg-3 mb-5">
        ...
        <div class="row mb-3 mb-lg-5">
            <div class="col text-center">
                <h3><strong>Nossos clientes</strong></h3>
            </div>
        </div>
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <table id="table" class="table table-striped py-lg-4" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($customers as $customer)
                                        <tr data-href="{{ route('visualizar_cliente', $customer->id) }}"
                                            style="cursor: pointer">
                                            <td>{{ $customer->name }}</td>
                                            <td>{{ $customer->responsible ? $customer->responsible->full_name : 'N/A' }}</td>
                                            <td>{{ $customer->email }}</td>
                                            <td>{{ $customer->events->count() }}</td>
                                            <td><span
                                                    class="badge bg-success">{{ $customer->origin === 0 ? 'Manual' : 'Outro' }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        @else
                            <p>Ainda não temos clientes cadastrados.</p>
                            <p>Adicione o primeiro cliente <a href="{{ route('novo_cliente') }}">clicando aqui.</a></p>
                        @endif
                    </div>
                </div>
            </div>
